...
export default function Cart() {
    const {clearSelection, cartItems, totalItems, totalPrice,displayModal } = useDataContext();
        ...
        return cartItems.map((item) => (
            <>
                <div className="cart-flexbox">
                    <div>
                        <div><p className="cart-item-title mb-8">{item.description}</p></div>
                        <div className="cart-item-details">
                            <p className="cart-quantity">{item.quantity}x</p>
                            <p className="cart-price text-gray-500">@ ${item.price.toFixed(2)}</p>
                            <p className="cart-item-total text-gray-900">${(item.price * item.quantity).toFixed(2)}</p>
                        </div>
                    </div>
                    <div class="w-4 h-4 flex items-center justify-center border border-gray-400 rounded-full" onClick={()=>{clearSelection(item.id)}} >
                        <svg xmlns="http://www.w3.org/2000/svg" width="8" height="8" fill="none" viewBox="0 0 10 10" class="text-gray-500">
    ...
    if (cartItems.length == 0) {
        ...
        return (
            <div className="cart-full">
                <h1 className="cart-h1 orange">Your Cart ({totalItems})</h1>
                {selectedItems()}
                <div className="cart-summary">
                    <p className="cart-total">Order Total</p>
                ...
                <button className="button-confirm text-white font-semibold" onClick={()=>{displayModal()}}>Confirm Order</button>
            </div>
        )
